...
Check-in and check-out enqueue a parent SMS in a server-side `sms_queue` table when the student has a parent phone on file. When `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, and `TWILIO_FROM_NUMBER` are set, the server sends that text immediately through Twilio and marks the queue row sent or failed. Inbound replies to the Twilio number POST to `/api/webhooks/sms` (HMAC-SHA1 `X-Twilio-Signature` verified against `TWILIO_AUTH_TOKEN`) and land in the same `messages` thread staff see in Admin. Queue inserts never block a scan. If Twilio is unset, a dedicated Android phone running `gateway-app/` can still poll the queue every 15 seconds with `GATEWAY_API_KEY` and send through the phone's SMS plan (3 retries, then `failed`).
