...
The demo is a separate KumonScan deployment pointed at its own Neon database. Isolation is physical: the demo deployment's `DATABASE_URL` names a database that no real center ever uses. The seed script wipes that database's students and sessions entirely (CASCADE through child tables), so it must never share a database with production, even now that production is multi-tenant.
...
- A logged-in demo admin can also `POST /api/demo/reset` to reset on demand mid-demo (`requireAdmin` session cookie).

## Seeded dataset

`Kumon of Demo Springs`: 18 students (17 active, 1 deactivated), fictional 555-01XX parent phones, student numbers 1–18, varied subjects (math/reading/both) and schedule days. Every run rebuilds, relative to the current time:
...
3. Seed once: `DEMO_MODE=true DATABASE_URL=<demo-url> node server/scripts/seed-demo-data.js`, or hit `GET /api/demo/reset` with `Authorization: Bearer <CRON_SECRET>`.
...
## Marketing site wiring (documented, not applied)

`marketing-site/` is on `main`. The intended change is still: keep the existing "Request a demo" lead-capture form, and add a "Try it now" link beside it pointing at the demo deployment URL (e.g. `https://demo.kumonscan.app`) with the credential `try-kumonscan` shown next to the link. Add alongside, do not replace: the lead form still captures contact intent; the live demo converts the impatient.

That link is not in the page yet. No demo Vercel project or hostname exists, and the public password should not be advertised until the isolated deployment is live. Wiring a 404 CTA on the marketing site is worse than leaving the form as the only path.
