...
function sendAction(action) {
  ...
  if (type === 'check-out') {
    // agent-2-pickup-auth: a picked_up_by payload came from the caregiver
    // pickup flow, not the plain desk checkout button — route it through the
    // same endpoint via caregiversApi so the pickup log still records who
    // collected the student, even when the action was queued offline.
    if (payload.picked_up_by) {
      return caregiversApi.checkOut({
