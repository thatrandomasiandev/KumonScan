...
describe('demo seed', () => {
  ...
  afterAll(async () => {
    await rows(realSql, `DELETE FROM students WHERE qr_code_value = 'REAL-SENTINEL-1'`);
  });
  ...
  it('is idempotent: two runs rebuild the same roster with no duplicates', async () => {
    ...
    const rosterOf = async () =>
      rows(
        demoSql,
        `SELECT first_name, last_name, qr_code_value, active, enrolled_subjects
         FROM students ORDER BY qr_code_value`
      );
    ...
    expect(await count(demoSql, 'SELECT COUNT(DISTINCT qr_code_value) AS count FROM students')).toBe(18);
  ...
  it('demo data is fully isolated from the real database', async () => {
    ...
    expect(
      await count(realSql, `SELECT COUNT(*) AS count FROM students WHERE qr_code_value LIKE 'DEMO-%'`)
    ).toBe(0);
    expect(await count(realSql, 'SELECT COUNT(*) AS count FROM sessions')).toBe(0);

    // Demo database: every student is DEMO-prefixed, and the real center's
    // sentinel never leaked in.
    expect(
      await count(demoSql, `SELECT COUNT(*) AS count FROM students WHERE qr_code_value NOT LIKE 'DEMO-%'`)
    ).toBe(0);
    expect(
      await count(
        demoSql,
        `SELECT COUNT(*) AS count FROM students WHERE qr_code_value = 'REAL-SENTINEL-1' OR last_name = 'Sentinel'`
      )
    ).toBe(0);
  }, 180_000);
