...
describe('parent progress digests', () => {
  ...
  it('cron then manual trigger for the same week produces exactly one digest_log row per student', async () => {
    ...
    // No delivery channel is installed on this branch: generated, not sent.
    expect(cronRes.body.counts.pending_no_channel).toBe(1);
    ...
    expect(manualRes.body.counts.pending_no_channel).toBe(0);
  ...
  it('the unique index rejects a second row for the same student and period at the database level', async () => {
    ...
    await insert.run(id, PERIOD.start, PERIOD.end, new Date().toISOString());
    await expect(
      insert.run(id, PERIOD.start, PERIOD.end, new Date().toISOString())
