...
function insertStudent({ first = 'Test', last = 'Student', subjects = 'both', days = null } = {}) {
...
describe('sessionHygiene', () => {
  ...
  it('closes a prior-day orphan at check-in + allowance', () => {
    const studentId = insertStudent({ first: 'Orphan', last: 'Prior' });
    ...
    const closed = closeStaleOpenSessions('2026-07-30');
    expect(closed).toBe(1);

    const row = db.prepare('SELECT * FROM sessions WHERE student_id = ?').get(studentId);
    expect(row.check_out_time).toBe(new Date(Date.parse(checkIn) + 30 * 60_000).toISOString());
    expect(row.duration_minutes).toBe(30);
  });

  it('collapses same-day duplicate open sessions down to the newest', () => {
    const studentId = insertStudent({ first: 'Dup', last: 'Open' });

    // Seed the historical bug state (two open rows) by temporarily dropping the unique index.
    db.exec('DROP INDEX IF EXISTS idx_one_open_session_per_student');
    db.prepare(
      `INSERT INTO sessions (student_id, check_in_time, subjects, allowance_minutes)
