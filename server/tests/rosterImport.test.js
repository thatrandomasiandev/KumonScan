...
describe('rosterImport', () => {
  ...
  it('does not touch schedule_days when CRM upload has no Days column', () => {
    seedStudent({ first: 'Ada', last: 'Lovelace', days: '["Tue","Thu"]' });
    ...
    const result = importRosterFromContent(csv);
  ...
  it('does not overwrite subjects when Subjects cell is blank for an existing student', () => {
    seedStudent({ first: 'Grace', last: 'Hopper', subjects: 'reading' });
  ...
  it('lands invalid names in skipped without aborting the batch', () => {
    const csv = [
      'First Name,Last Name,Subjects',
      ',MissingFirst,math',
