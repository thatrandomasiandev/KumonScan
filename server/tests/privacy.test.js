...
describe('privacy & data handling', () => {
  ...
  describe('audit trail on write endpoints', () => {
    it('captures staff student create/update with actor and timestamp', async () => {
      ...
      // Scoped by entity_id, not just entity_type: auditTrailMiddleware is
      // global, so other test files creating/updating students against the
      // same shared default center concurrently would otherwise inflate
      // this count.
      await vi.waitFor(
        async () => {
          const rows = await auditRows({ entity_type: 'student', entity_id: String(create.body.id) });
    ...
    it('captures session writes from check-in/check-out', async () => {
      ...
      // Scoped by entity_id for the same reason as the student test above.
      await vi.waitFor(
        async () => {
          const rows = await auditRows({
            entity_type: 'session',
            entity_id: String(checkIn.body.session.id),
          });
          expect(rows.length).toBe(2);
        },
        { timeout: 5000 }
    ...
    it('attributes kiosk registration to a system actor', async () => {
      ...
      // Scoped by entity_id, not just action: 'create' is the broadest
      // possible filter (matches every entity type), so it's the most
      // exposed to other concurrently-running test files' writes.
      await vi.waitFor(
        async () => {
          const rows = await auditRows({ action: 'create', entity_id: String(res.body.student_id) });
          expect(rows.length).toBe(1);
        },
        { timeout: 5000 }
      );

      const [row] = await auditRows({ action: 'create', entity_id: String(res.body.student_id) });
