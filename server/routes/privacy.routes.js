...
async function loadStudent(id, centerId) {
  const studentId = Number(id);
  if (!Number.isInteger(studentId) || studentId < 1) return null;
  return get('SELECT * FROM students WHERE id = ? AND center_id = ?', [studentId, centerId]);
}
...
router.get('/admin/privacy/retention', requireAdmin, async (req, res) => {
  try {
    const [policies, purgeableTables] = await Promise.all([
      listRetentionPolicies(req.center.id),
      listPurgeableTables(),
    ]);
...
router.put('/admin/privacy/retention', requireAdmin, async (req, res) => {
  ...
  try {
    if (retain_days == null) {
      const result = await clearRetentionPolicy(table, req.center.id);
      return res.json({ ok: true, ...result, policies: await listRetentionPolicies(req.center.id) });
    }
    ...
    const policy = await setRetentionPolicy(table, retain_days, req.center.id);
    res.json({ ok: true, ...policy, policies: await listRetentionPolicies(req.center.id) });
  } catch (err) {
 ...
 */
router.post('/admin/privacy/purge-expired', requireAdmin, async (req, res) => {
  try {
    const policies = await listRetentionPolicies(req.center.id);
    ...
    const result = await purgeExpiredData({ centerId: req.center.id });
...
router.get('/admin/privacy/export-student-data/:id', requireAdmin, async (req, res) => {
  try {
    ...
    const student = await loadStudent(req.params.id, req.center.id);
    if (!student) {
...
router.delete('/admin/students/:id/purge', requireAdmin, async (req, res) => {
  try {
    ...
    await run('DELETE FROM students WHERE id = ? AND center_id = ?', [student.id, req.center.id]);
    deleted.students = 1;
...
router.patch('/admin/students/:id/consent', requireAdmin, async (req, res) => {
  try {
    ...
    await run('UPDATE students SET contact_consent_on_file = ? WHERE id = ? AND center_id = ?', [
      value ? 1 : 0,
      student.id,
      req.center.id,
    ]);
