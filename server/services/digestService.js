 ...
 */
async function claimDigestRow(studentId, periodStart, periodEnd, nowIso) {
  const inserted = await db
    .prepare(
      `INSERT INTO digest_log (student_id, period_start, period_end, channel, status, created_at)
       VALUES (?, ?, ?, 'none', 'pending', ?)
       ON CONFLICT (student_id, period_start, period_end) DO NOTHING
       RETURNING id`
    )
    .run(studentId, periodStart, periodEnd, nowIso);
...
export async function sendDigestForStudent(
  ...
  const claimed = await claimDigestRow(student.id, periodStart, periodEnd, sqlNow());
  if (!claimed) {
...
export async function runDigestBatch({
  ...
  let students = await db
    .prepare('SELECT * FROM students WHERE active = 1 ORDER BY id ASC')
    .all();
  if (studentIds) {
...
export async function listDigestLog({ studentId = null, limit = 100 } = {}) {
  await ensureDigestSchema();

  const params = [];
  let where = '';
  if (studentId != null) {
    where = 'WHERE dl.student_id = ?';
