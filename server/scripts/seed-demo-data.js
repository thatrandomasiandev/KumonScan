 ...
 * database (multi-tenancy has not landed on main, so isolation is at the
 * database level, never row level). This script is the only writer of demo
 * state; the nightly cron (/api/demo/reset) and manual CLI runs both call
 * resetAndSeedDemo().
 *
 ...
 * (names, QR codes, schedules) with timestamps relative to "now".
 */
...
export const DEMO_QR_PREFIX = 'DEMO-';
 ...
 * Phones use the reserved fictional 555-01XX range; QR codes are DEMO-prefixed
...
function demoQr(index) {
  return `${DEMO_QR_PREFIX}STU-${String(index + 1).padStart(3, '0')}`;
}
...
function buildTodaySessionRows(now, studentIdByQr, rand) {
  const rows = [];
  ...
  for (const index of CHECKED_IN_INDEXES) {
    ...
    rows.push([studentId, checkIn.toISOString(), null, null, entry.subjects, allowance]);
  }
...
export async function resetAndSeedDemo({ now = new Date() } = {}) {
  ...
  // Children of sessions first (FK), then students.
  ...
  const students = await insertRows(
    ...
    buildStudentRows(now)
  );
  const studentIdByQr = new Map(students.map((s) => [s.qr_code_value, s.id]));

  const sessionRows = [
    ...buildHistorySessionRows(now, studentIdByQr, rand),
    ...buildTodaySessionRows(now, studentIdByQr, rand),
  ];
  await insertRows(
    'sessions',
